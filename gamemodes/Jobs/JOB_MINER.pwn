...
Player_GiveLog(playerid, type)
{
	...
	Info(playerid, "You can press "GREEN_E"'Y' "WHITE_E"to drop your ore.");
	return 1;
}
...
CMD:ore(playerid, params[])
{
	if(pData[playerid][pJob] == 5 || pData[playerid][pJob2] == 5)
	{
		...
		{
			//if(StockCrateCompo >= 100) return Error(playerid, "Gudang crate component penuh!");
			...
			if(!IsATruck(vehicleid)) return Error(playerid, "Vehicle isn't a mining vehicle.");
			if(!IsPlayerInRangeOfPoint(playerid, 4.0, 273.7129,1358.4484,10.5859)) return Error(playerid, "You're not near a miner warehouse.");
			...
			StockCrateCompo += LogStorage[vehicleid][0];
			StockCrateCompo += LogStorage[vehicleid][1];
			//GasOil += LogStorage[vehicleid][1] + 100;
			if(StockCrateCompo >= 100) StockCrateCompo = 100;
