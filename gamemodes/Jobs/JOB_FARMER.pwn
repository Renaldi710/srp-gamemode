...
//Drug 333.6938,1124.4623,1083.8903
...
function HarvestPlant(playerid)
{
	if(pData[playerid][pHarvestID] != -1)
	{
		...
		if(pData[playerid][pActivityTime] >= 100)
		{
			if(PlantData[id][PlantType] == 1)
			{
				Inventory_Add(playerid, "Potato", 19577, kg);
				Info(playerid, "Anda mendapatkan hasil panen "YELLOW"potato/kentang "WHITE_E"seberat "GREEN_E"%d kg.", kg);
			}
			else if(PlantData[id][PlantType] == 2)
			{
				Inventory_Add(playerid, "Wheat", 2247, kg);
				Info(playerid, "Anda mendapatkan hasil panen "YELLOW"wheat/gandum "WHITE_E"seberat "GREEN_E"%d kg.", kg);
			}
			else if(PlantData[id][PlantType] == 3)
			{
				Inventory_Add(playerid, "Orange", 19574, kg);
				Info(playerid, "Anda mendapatkan hasil panen "YELLOW"orange/jeruk "WHITE_E"seberat "GREEN_E"%d kg.", kg);
			}
			else if(PlantData[id][PlantType] == 4)
			{
				Inventory_Add(playerid, "Marijuana", 1578, kg);
				Info(playerid, "Anda mendapatkan hasil panen "YELLOW"marijuana/ganja "WHITE_E"seberat "GREEN_E"%d kg.", kg);
			}
			
			pData[playerid][pScoreFarmer] += kg;
...
CMD:plant(playerid, params[])
{
	...
	if(!strcmp(params, "plant", true))
	{
		if(pData[playerid][pJob] == 7 || pData[playerid][pJob2] == 7)
		{
			...
			format(tstr, sizeof(tstr), ""WHITE_E"My Seed: "GREEN_E"%d", Inventory_Count(playerid, "Seed"));
	...
	else if(!strcmp(params, "harvest", true))
	{
		if(pData[playerid][pJob] == 7 || pData[playerid][pJob2] == 7)
		{
			...
			if(pData[playerid][pSkillFarmer] == 1)
			{
				if(Inventory_Count(playerid, "Wheat") >= 150) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Orange") >= 150) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Marijuana") >= 150) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Potato") >= 150) return Error(playerid, "Your inventory is full"); 
			}
			if(pData[playerid][pSkillFarmer] == 2)
			{
				if(Inventory_Count(playerid, "Wheat") >= 200) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Orange") >= 200) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Marijuana") >= 200) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Potato") >= 200) return Error(playerid, "Your inventory is full");
			}
			if(pData[playerid][pSkillFarmer] == 3)
			{
				if(Inventory_Count(playerid, "Wheat") >= 250) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Orange") >= 250) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Marijuana") >= 250) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Potato") >= 250) return Error(playerid, "Your inventory is full");
			}
			if(pData[playerid][pSkillFarmer] == 4)
			{
				if(Inventory_Count(playerid, "Wheat") >= 300) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Orange") >= 300) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Marijuana") >= 300) return Error(playerid, "Your inventory is full"); 
				if(Inventory_Count(playerid, "Potato") >= 300) return Error(playerid, "Your inventory is full"); 
			}
	...
	else if(!strcmp(params, "sell", true))
	{
		...
		new potato = Inventory_Count(playerid, "Potato") * PotatoPrice,
		wheat = Inventory_Count(playerid, "Wheat") * WheatPrice,
		orange = Inventory_Count(playerid, "Orange") * OrangePrice;
		
		new total = Inventory_Count(playerid, "Potato") + Inventory_Count(playerid, "Wheat") + Inventory_Count(playerid, "Orange");
		new pay = potato + wheat + orange;
		
		if(total < 1) return Error(playerid, "You dont have plant!");
		GivePlayerMoneyEx(playerid, pay);

		StockCratePlant += total;
		if(StockCratePlant >= 100) StockCratePlant = 100;
		...
		Inventory_Remove(playerid, "Potato", 0);
		Inventory_Remove(playerid, "Wheat", 0);
		Inventory_Remove(playerid, "Orange", 0);
		Info(playerid, "You selling "YELLOW"%d kg "WHITE_E"all plant to "GREEN_E"%s", total, FormatMoney(pay));
	}
	return 1;
}
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.5, 333.6938,1124.4623,1083.8903))
	...
	Info(playerid, ""YELLOW"%d kg "WHITE_E"marijuana to "GREEN_E"%s", total, FormatMoney(pay));
...
CMD:cook(playerid, params[])
{
	...
	if(!strcmp(params, "snack", true))
	{
		...
		Info(playerid, "Anda sedang memasak makanan dengan "YELLOW"5 food!");
...
function CookingFood(playerid, type)
{
	...
	if(!pData[playerid][pCooking]) return 0;
	if(pData[playerid][pInHouse] != -1)
	{
		if(pData[playerid][pActivityTime] >= 100)
		{
