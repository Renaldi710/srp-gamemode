...
CreateJoinMechPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, 2329.8792,-2315.4001,13.5469, -1);
	format(strings, sizeof(strings), "[MECH JOBS]\n{ffffff}Jadilah Pekerja Mekanik disini\n{7fff00}/getjob /accept job");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, 2329.8792,-2315.4001,13.5469, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // Taxi
}
...
IsAtMech(playerid)
{
	...
	{
		// mechanic center los santos
		if(IsPlayerInRangeOfPoint(playerid, 15.0, 2200.5042,-2188.5366,13.5869) || 
			IsPlayerInRangeOfPoint(playerid, 15.0, 2194.1245,-2194.7500,13.5869) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, 2187.9380,-2200.7195,13.5869) || 
			IsPlayerInRangeOfPoint(playerid, 15.0, 2181.5745,-2206.8611,13.5869) ||
			//IsPlayerInRangeOfPoint(playerid, 15.0, 298.59, -248.54, 1.60) || IsPlayerInRangeOfPoint(playerid, 10.0, 266.20, -253.32, 1.59)
			IsPlayerInRangeOfPoint(playerid, 30, 2199.5164,-2238.0964,13.5669) || 
			IsPlayerInRangeOfPoint(playerid, 15.0, 2206.0112,-2232.1589,13.5669) || 
			IsPlayerInRangeOfPoint(playerid, 15.0, 2212.1182,-2225.6331,13.5669) /*||
			IsPlayerInRangeOfPoint(playerid, 15.0, -1854.5592,16.4390,15.2907) || 
			IsPlayerInRangeOfPoint(playerid, 15.0, -1854.6669,24.9225,15.2907) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, -1854.7661,37.5907,15.2907) || 
			IsPlayerInRangeOfPoint(playerid, 15.0, -1854.4707,46.6991,15.2907) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, -1854.2528,55.4863,15.2907) || 
			IsPlayerInRangeOfPoint(playerid, 15.0, -1854.0594,64.6941,15.2907) ||
			IsPlayerInRangeOfPoint(playerid, 15.0, -1853.6832,101.4869,15.2025) || 
			IsPlayerInRangeOfPoint(playerid, 15.0, -1855.7808,107.5902,15.2025) ||
			IsPlayerInRangeOfPoint(playerid, 20.0,-81.95, -1571.55, 2.61) || 
			IsPlayerInRangeOfPoint(playerid, 20.0, 1264.85, -2026.17, 59.53)*/)
		{
			return 1;
		}
...
{
	if(!IsAtMech(playerid)) return Error(playerid, "Anda tidak berada di mechanic center atau workshop");
	...
	{		
		if(pData[playerid][pMechDuty] == 0)
		{
			...
			SendClientMessageToAllEx(COLOR_GREEN, "[MECH]"WHITE_E" %s is now on Mehcanic Duty. Type \"/call 111\" to call a taxi!", ReturnName(playerid));
		}
		else
		{
...
CMD:service(playerid, params[])
{
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		if(pData[playerid][pMechVeh] == INVALID_VEHICLE_ID)
		{
			...
			foreach(new i : PVehicles)
			{
				if(pvData[i][cVeh] == vehicleid)
				{
					...
					Vehicle(GetVehicleOwner(i),"Your "YELLOW"%s "WHITE"is on process checked by "LIGHTGREEN"%s", GetVehicleModelName(pvData[i][cModel]), ReturnName(playerid));
				}
			}
		...
		if(GetNearestVehicleToPlayer(playerid, 3.5, false) == pData[playerid][pMechVeh])
		{
			...
			if(pData[playerid][pSkillMecha] > 3)
			{
				...
				foreach(new id : Workshop)
				{
					if(IsPlayerInRangeOfPoint(playerid, 50.0, wsData[id][wX], wsData[id][wY], wsData[id][wZ]))
					{
						if(IsWorkshopOwner(playerid, id) || IsWorkshopEmploye(playerid, id))
						{
							...
							format(Dstring, sizeof(Dstring), "%sBody Upgrade\t950 Component\n", Dstring);
							format(Dstring, sizeof(Dstring), "%sEngine Upgrade\t1000 Component\n", Dstring);
						}
					}
				}
			}
...
function CheckCar(playerid, vehicleid)
{
	...
	if(!pData[playerid][pMechanic]) return 0;
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
...
function EngineFix(playerid, vehicleid)
{
	...
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
		{
			if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
			{
				if((pv = Vehicle_GetID(vehicleid) != -1))
				{
					SetVehicleMaxHealth(pv);
				}
				else
				{
		...
		if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
		{
			if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
			{
				foreach(new ii : PVehicles)
				{
					if(vehicleid == pvData[ii][cVeh])
					{
						pvData[ii][cEngineUpgrade] = 1;
					}
				}
				
				InfoTD_MSG(playerid, 8000, "Engine upgrade done!");
				pData[playerid][pMechVeh] = vehicleid;
				KillTimer(pData[playerid][pMechanic]);
				pData[playerid][pActivityTime] = 0;
				HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
...
function BodyUpgrade(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(!pData[playerid][pMechanic]) return 0;
		...
		if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
		{
			if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
			{
				foreach(new ii : PVehicles)
				{
					if(vehicleid == pvData[ii][cVeh])
					{
						pvData[ii][cBodyUpgrade] = 1;
					}
				}
				
				InfoTD_MSG(playerid, 8000, "Body upgrade done!");
				pData[playerid][pMechVeh] = vehicleid;
				KillTimer(pData[playerid][pMechanic]);
				pData[playerid][pActivityTime] = 0;
				HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
			...
			{
				Error(playerid, "Body Upgrade! Anda tidak berada didekat kendaraan tersebut!");
				KillTimer(pData[playerid][pMechanic]);
				HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
				PlayerTextDrawHide(playerid, ActiveTD[playerid]);
				pData[playerid][pActivityTime] = 0;
