...
{
	if(pData[playerid][pProductTime] > 0) return Error(playerid, "Anda harus menunggu "GREY2_E"%d "WHITE_E"detik untuk bisa bekerja kembali.", pData[playerid][pProductTime]);
	if(pData[playerid][pJob] == 6 || pData[playerid][pJob2] == 6)
	{
		...
		|| IsPlayerInRangeOfPoint(playerid, 2.0, -250.88, -2143.23, 29.32) || IsPlayerInRangeOfPoint(playerid, 2.0, -245.74, -2141.65, 29.32))
		{
			...
			if(type == 1)
			{
				...
				if(Inventory_Count(playerid, "Food") < 40) return Error(playerid, "Food tidak cukup!(Minimal: 40).");
			...
			{
				if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
				if(Inventory_Count(playerid, "Material") < 40) return Error(playerid, "Material tidak cukup!(Butuh: 40).");
				if(pData[playerid][CarryProduct] != 0) return Error(playerid, "Anda sedang membawa sebuah product");
			...
			else if(type == 3)
			{
				...
				if(pData[playerid][CarryProduct] != 0) return Error(playerid, "Anda sedang membawa sebuah product");
				Inventory_Remove(playerid, "Material", 40);
				Inventory_Count(playerid, "Component", 20);
...
CMD:sellproduct(playerid, params[])
{
	...
	if(Product >= 1500) return Error(playerid, "Kamu tidak bisa menjual product sementara waktu, gunakan /dropproduct.");
...
CMD:dropproduct(playerid, params[])
{
	if(pData[playerid][CarryProduct] == 0) return Error(playerid, "Kamu sedang tidak memegang apapun.");
	RemovePlayerAttachedObject(playerid, 9);
	SetPlayerSpecialAction(playerid, SPECIAL_ACTION_NONE);
