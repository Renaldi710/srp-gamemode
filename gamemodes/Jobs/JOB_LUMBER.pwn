CreateJoinLumberPoint()
{
	//JOBS
	new strings[128];
	CreateDynamicPickup(1239, 23, -1438.6384,-1544.8308,102.2578, -1, -1, -1, 5.0);
	format(strings, sizeof(strings), "[LUMBER JOBS]\n{ffffff}Jadilah Pekerja Lumber disini\n{7fffd4}/getjob /accept job");
	CreateDynamic3DTextLabel(strings, COLOR_YELLOW, -1438.6384,-1544.8308,102.2578, 3.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // lumber
}
...
CMD:lumber(playerid, params[])
{
	if(pData[playerid][pJob] == 3 || pData[playerid][pJob2] == 3)
	{
		...
		if(isnull(params)) return Usage(playerid, "/lum [cut/take/load/unload/pickup]");
		...
		else if(!strcmp(params, "load", true))
		{
			...
			Player_RemoveLumber(playerid);
			Custom(playerid, "JOB", "You've loaded 1 log, Total log stored "YELLOW_E"%d/%d", pvData[carid][cLumber], LUMBER_LIMIT); 
		}
		else if(!strcmp(params, "unload")) 
		{
			if(!IsPlayerInRangeOfPoint(playerid, 3.0, -1425.8391,-1529.2734,102.2214)) 
				return Error(playerid, "You're not near a lumber warehouse.");
			...
			if((carid = Vehicle_Nearest2(playerid)) != -1)
    		{
				if(IsValidVehicle(pvData[carid][cVeh]))
				{
					if(pvData[carid][cLumber] < 1) return Error(playerid, "This vehicle doesn't have any logs.");

					price = pvData[carid][cLumber] * LumberPrice;

					GivePlayerMoneyEx(playerid, price);
					Server_MinMoney(price);
					StockCrateLumber += 1;
					if(StockCrateLumber >= 100) StockCrateLumber = 100;
					...
					Info(playerid, "You've Sold "YELLOW"%d "WHITE"lumber for "GREEN_E"%s.", pvData[carid][cLumber], FormatMoney(price));

					pData[playerid][pLumberTime] = 600;
					pvData[carid][cLumber] = 0;
					...
					if(IsValidVehicle(pvData[carid][cVeh]))
					{
						...
						{
							if(IsValidDynamicObject(LumberObjects[pvData[carid][cVeh]][i]))
							{
								DestroyDynamicObject(LumberObjects[pvData[carid][cVeh]][i]);
								LumberObjects[pvData[carid][cVeh]][i] = -1;
								break;
							}
						}
					}
...
Player_GiveLumber(playerid)
{
	...
	Info(playerid, "You can press "GREEN_E"'Y' "WHITE_E"to drop your log.");
	return 1;
}
