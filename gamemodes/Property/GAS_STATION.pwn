 	...
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name_int(i, "bisnis", gsData[gsid][gsBisnis]);
...
{
	new cQuery[512];
	format(cQuery, sizeof(cQuery), "UPDATE gstations SET stock='%d', bisnis='%d', posx='%f', posy='%f', posz='%f' WHERE id='%d'",
	gsData[gsid][gsStock],
	gsData[gsid][gsBisnis],
	gsData[gsid][gsPosX],
	gsData[gsid][gsPosY],
	gsData[gsid][gsPosZ],
	gsid
...
CMD:creategs(playerid, params[])
{
	...
	new stok, bisnis;
	if(sscanf(params, "dd", stok, bisnis)) return Usage(playerid, "/creategs [bisnisid] [stock - max: 10000/liters]");
	
	if(stok < 1 || stok > 10000) return Error(playerid, "Invagsid stok.");

	if(!Iter_Contains(Bisnis, bisnis))
		return Error(playerid, "Invalid Bisnis ID");

	if(bData[bisnis][bType] != 2)
		return Error(playerid, "Bisnis harus bertipe market");
	
	GetPlayerPos(playerid, gsData[gsid][gsPosX], gsData[gsid][gsPosY], gsData[gsid][gsPosZ]);
	gsData[gsid][gsStock] = stok;
	gsData[gsid][gsBisnis] = bisnis;
    GStation_Refresh(gsid);
	Iter_Add(GStation, gsid);
...
CMD:editgs(playerid, params[])
{
    ...
    if(sscanf(params, "ds[24]S()[128]", gsid, type, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} location, stock, bisnis, delete");
        return 1;
    }
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has adjusted the location of gs ID: %d.", pData[playerid][pAdminname], gsid);
    }
    else if(!strcmp(type, "stock", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has set gs ID: %d stock to %d.", pData[playerid][pAdminname], gsid, stok);
    }
	else if(!strcmp(type, "bisnis", true))
    {
        new bisnis;

        if(sscanf(string, "d", bisnis))
            return Usage(playerid, "/editgs [id] [bisnis] [bisnis id]");
        ...
        SendAdminMessage(X11_TOMATO, "%s has set gs ID: %d bisnis to %d.", pData[playerid][pAdminname], gsid, bisnis);
    }
    else if(!strcmp(type, "delete", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has delete gs ID: %d.", pData[playerid][pAdminname], gsid);
    }
    return 1;
}
...
{
	if(!pData[playerid][pFillTime]) return 0;
	foreach(new gsid : GStation)
	{
		...
		else
		{
			...
			SetVehicleFuel(GetPlayerVehicleID(playerid), old + 85);
...
function DeleteGasStation(playerid, bid)
{
	foreach(new gsid : GStation) {
		if(gsData[gsid][gsBisnis] == bid) {
			new gsQuery[128];
			DestroyDynamic3DTextLabel(gsData[gsid][gsLabel]);
			DestroyDynamicPickup(gsData[gsid][gsPickup]);
			gsData[gsid][gsPosX] = 0;
			...
			gsData[gsid][gsPickup] = -1;
			Iter_Remove(GStation, gsid);
			mysql_format(g_SQL, gsQuery, sizeof(gsQuery), "DELETE FROM gstations WHERE id=%d", gsid);
			mysql_tquery(g_SQL, gsQuery);
			SendAdminMessage(X11_TOMATO, "%s has delete gs ID: %d.", pData[playerid][pAdminname], gsid);
		}
	}
	return 1;
