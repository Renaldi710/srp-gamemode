...
Bisnis_BuyMenu(playerid, bizid)
{
    ...
    switch(bData[bizid][bType])
    {
        case 1:
        {
            ...
            ShowPlayerDialog(playerid, BISNIS_BUYPROD, DIALOG_STYLE_TABLIST_HEADERS, ReplaceString(bData[bizid][bName]), string, "Buy", "Cancel");
        }
        case 2:
        {
...
Bisnis_ProductMenu(playerid, bizid)
{
    ...
    switch (bData[bizid][bType])
    {
		...
		case 5: //electronic
        {
            ...
            ShowPlayerDialog(playerid, BISNIS_EDITPROD, DIALOG_STYLE_TABLIST_HEADERS, "Business: Modify Item", string, "Buy", "Cancel");
		}
    }
    return 1;
}
...
Bisnis_Type(bisid)
{
	...
	{
		bData[bisid][bIntposX] = 286.148986;
		bData[bisid][bIntposY] = -40.644397;
		bData[bisid][bIntposZ] = 1001.515625;
		bData[bisid][bIntposA] = 0;
		bData[bisid][bInt] = 1;
	}
	if(bData[bisid][bType] == 5) // electronic
	{
		bData[bisid][bIntposX] = -2240.3938;
		bData[bisid][bIntposY] = 137.3993;
		bData[bisid][bIntposZ] = 1035.4141;
		bData[bisid][bIntposA] = 270.2590;
		bData[bisid][bInt] = 6;
	}
}
...
Bisnis_Refresh(id)
{
    if(Iter_Contains(Bisnis, id))
    {
		...
		if(IsValidDynamicCP(bData[id][bCP][0]))
			DestroyDynamicCP(bData[id][bCP][0]);

		if(IsValidDynamicCP(bData[id][bCP][1]))
			DestroyDynamicCP(bData[id][bCP][1]);
		...
		{
			format(string, sizeof(string), "[ID: %d]\n"WHITE"Bisnis Name: "GREEN"%s\n"WHITE_E"Bisnis Owner: "YELLOW"%s", id, bData[id][bName], bData[id][bOwner]);
		...
		bData[id][bLabel] = CreateDynamic3DTextLabel(string, COLOR_GREY, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ]+0.5, 2.5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
		
		bData[id][bCP][0] = CreateDynamicCP(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 1.7, -1, -1, _, 3.0);
		bData[id][bCP][1] = CreateDynamicCP(bData[id][bIntposX], bData[id][bIntposY], bData[id][bIntposZ], 1.7, id, bData[id][bInt], _, 3.0);
		if(bData[id][bType] == 1)
		{
		...
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 48, -1, -1, -1, -1, 70.0);
		}
		else
		{
			DestroyDynamicMapIcon(bData[id][bMap]);
...
CMD:createbisnis(playerid, params[])
{
	...
	new price, type;
	if(sscanf(params, "dd", price, type)) return Usage(playerid, "/createbisnis [price] [type, 1.Fastfood 2.Market 3.Clothes 4.Equipment 5.Electronic]");
...
CMD:gotobisnis(playerid, params[])
{
	...
	Info(playerid, "You has teleport to bisnis id %d", bid);
...
CMD:editbisnis(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has adjusted the location of bisnis ID: %d.", pData[playerid][pAdminname], bid);
    }
    else if(!strcmp(type, "interior", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has adjusted the interior spawn of bisnis ID: %d.", pData[playerid][pAdminname], bid);
    }
    else if(!strcmp(type, "locked", true))
    {
        ...
        if(locked) {
            SendAdminMessage(X11_TOMATO, "%s has locked bisnis ID: %d.", pData[playerid][pAdminname], bid);
        }
        else {
            SendAdminMessage(X11_TOMATO, "%s has unlocked bisnis ID: %d.", pData[playerid][pAdminname], bid);
        }
    }
    else if(!strcmp(type, "price", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the price of bisnis ID: %d to %d.", pData[playerid][pAdminname], bid, price);
    }
	else if(!strcmp(type, "type", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the type of bisnis ID: %d to %d.", pData[playerid][pAdminname], bid, btype);
    }
	else if(!strcmp(type, "product", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the product of bisnis ID: %d to %d.", pData[playerid][pAdminname], bid, prod);
    }
	else if(!strcmp(type, "money", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the money of bisnis ID: %d to %s.", pData[playerid][pAdminname], bid, FormatMoney(money));
    }
	else if(!strcmp(type, "restock", true))
    {
		...
		if(prod == 0)
		{
			...
			Bisnis_Save(bid);
			Bisnis_Refresh(bid);
			SendAdminMessage(X11_TOMATO, "%s has adjusted the restock of bisnis ID: %d to disable.", pData[playerid][pAdminname], bid);
		}
		else if(prod == 1)
		{
			...
			Bisnis_Save(bid);
			Bisnis_Refresh(bid);
			SendAdminMessage(X11_TOMATO, "%s has adjusted the restock of bisnis ID: %d to enable.", pData[playerid][pAdminname], bid);
		}
    ...
    else if(!strcmp(type, "owner", true))
    {
        ...
        Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the owner of bisnis ID: %d to %s", pData[playerid][pAdminname], bid, owners);
    }
    else if(!strcmp(type, "reset", true))
    {
		...
		Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(X11_TOMATO, "%s has reset bisnis ID: %d.", pData[playerid][pAdminname], bid);
    }
	else if(!strcmp(type, "point", true))
    {
		...
		Bisnis_Save(bid);
		Bisnis_Refresh(bid);
        SendAdminMessage(X11_TOMATO, "%s has edit bisnis point ID: %d.", pData[playerid][pAdminname], bid);
    }
	else if(!strcmp(type, "delete", true))
    {
        ...
        DestroyDynamicCP(bData[bid][bCP][0]);
		DestroyDynamicCP(bData[bid][bCP][1]);
		...
		if(bData[bid][bType] == 2) {
			CallLocalFunction("DeleteGasStation", "dd", playerid, bid);
		}
        ...
        SendAdminMessage(X11_TOMATO, "%s has delete bisnis ID: %d.", pData[playerid][pAdminname], bid);
	}
    return 1;
}
/*
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{FF0000}North Country {0000FF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
