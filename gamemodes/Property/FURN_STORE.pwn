...
function FurnStore_Load()
{
	...
	printf("*** [NPRP Database: Loaded] furnstore data (%d count).", cache_num_rows());
	return 1;
}
...
FurnStore_Save(index)
{
	...
	{
		new query[500];
		format(query, sizeof(query), "UPDATE `furnstore` SET `ownername` ='%s', `name` ='%e', `x` =%.1f, `y` =%.1f, `z` =%.1f, `i_x` =%.1f, `owner` =%d, `price`=%d, `vault` =%d, `employe1` =%d, `employe2` =%d, `employe3` =%d, `i_y` =%.1f, `i_z` =%.1f, `seal`=%d WHERE `id` = %d",
	...
	if(Iter_Contains(FurnStore, index))
	{
		...
		if(storeData[index][storeOwner])  {
			if (storeData[index][storeSeal]) {
				storeData[index][storeLabel] = CreateDynamic3DTextLabel(sprintf(""GRAY"[ID: %d]\n%s\n"WHITE"Owned by: "YELLOW"%s\n"WHITE"This furniture store is sealed by "RED"authority", index, storeData[index][storeName], storeData[index][storeOwnerName]), X11_LIGHTBLUE, storeData[index][storePos][0], storeData[index][storePos][1], storeData[index][storePos][2]+0.5, 10, _, _, 1, 0, 0);
			} else {
				storeData[index][storeLabel] = CreateDynamic3DTextLabel(sprintf(""GRAY"[ID: %d]\n%s\n"WHITE"Owned by: "YELLOW"%s", index, storeData[index][storeName], storeData[index][storeOwnerName]), X11_LIGHTBLUE, storeData[index][storePos][0], storeData[index][storePos][1], storeData[index][storePos][2]+0.5, 10, _, _, 1, 0, 0);
			}
		}
		else {
			storeData[index][storeLabel] = CreateDynamic3DTextLabel(sprintf(""GRAY"[ID: %d]\n"GREEN"This Furniture Store is for sale\n"WHITE"Price: "YELLOW"%s\n"WHITE"Type /buy to buy this", index, FormatMoney(storeData[index][storePrice])), X11_LIGHTBLUE, storeData[index][storePos][0], storeData[index][storePos][1], storeData[index][storePos][2]+0.5, 10, _, _, 1, 0, 0);
		}
...
{
	if(!pData[playerid][IsLoggedIn] || pData[playerid][pID] == 0)
		return 0;
...
GetDataByIndex(index, type = 0)
{
	...
	getname = mysql_query(g_SQL, sprintf("SELECT `%s` FROM `players` WHERE `reg_id`='%d'", !type ? ("username") : ("last_login"), index));
	if(cache_num_rows()) {
		if(!type)
			cache_get_value(0, "username", name);
		else {
			cache_get_value_int(0, "last_login", seen);
			format(name, MAX_PLAYER_NAME, "%s", seen);
		}
	}
...
timer productionTimers[1000](playerid)
{
	...
	if(keys == KEY_NO && startProduce[playerid] && IsPlayerInDynamicArea(playerid, production) && GetPVarInt(playerid, "lagiedit") == 2)
	{
		if(++productionCount[playerid] >= 20)
		{
			...
			Info(playerid, "Production successfull.");
			DeletePVar(playerid, "lagiedit");
		}
		ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.1, 0, 0, 0, 0, 0, 1);
		InfoTD_MSG(playerid, 3000, sprintf("~b~~h~Memproduksi furniture: ~w~%d/~r~~h~20",productionCount[playerid]));
	}
}
...
CMD:addfurnstore(playerid, params[])
{
	...
	Info(playerid, "Furniture store id %d telah di buat, gunakan /editfurnstore untuk mengubah sesuatu.", id);
	return 1;
}
...
CMD:destroyfurnstore(playerid, params[])
{
	...
	Info(playerid, "Kamu telah menghapus furnstore id %d", id);
	return 1;
}
...
CMD:editfurnstore(playerid, params[])
{
	...
	else if(!strcmp(opsi, "price"))
	{
		...
		Info(playerid, "You've update price for furnstore id #%d to %d.", id, price);
	...
	else if(!strcmp(opsi, "vault"))
	{
		...
		Info(playerid, "You've update vault for furnstore id #%d to %s.", id, FormatMoney(vault));
	...
	else if(!strcmp(opsi, "resetemployee"))
	{
		...
		Info(playerid, "You've reset employee for furnstore id #%d", id);
		FurnStore_Save(id);
	}
	else if(!strcmp(opsi, "sell"))
	{
		...
		Info(playerid, "You've selling furnstore id #%d.", id);
	...
	else if(!strcmp(opsi, "employee"))
	{
		...
		Info(playerid, "You've edit employee for furnstore id #%d", id);
	}
...
CMD:gotofurnstore(playerid, params[])
{
	...
	Info(playerid, "You've teleported to furn store id %d.", index);
	return 1;
}
...
/*Dialog:SelectFurnObject(playerid, response, listitem, inputtext[])
{
		if (response)
		{
		Info(playerid, "listitem: %d , model: %s", listitem, inputtext);
	}
	return 1;
}
*/
		...
		switch(listitem)
		{
			case 0: {
				...
				if (pData[playerid][pJob] != 10 || pData[playerid][pJob2] != 10)
					return Error(playerid, "You must to be a Builder to restocking furniture.");
...
Dialog:FurnStoreProduct(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		Info(playerid, "Pergilah ke area produksi untuk memulainya.");
	}
...
Dialog:FurnObjectName(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		FurnObject_Refresh(ManageFurnObject[playerid]);
		FurnObject_Save(ManageFurnObject[playerid]);
		Info(playerid, "Nama telah di ganti menjadi: "GREEN"%s", FurnStore[ManageFurnObject[playerid]][furnName]);
	}
	return 1;
}
...
Dialog:FurnObjectPrice(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		FurnObject_Refresh(ManageFurnObject[playerid]);
		FurnObject_Save(ManageFurnObject[playerid]);
		Info(playerid, "Harga telah di ganti menjadi: "YELLOW"%s", FormatMoney((FurnStore[ManageFurnObject[playerid]][furnPrice])));
	}
	return 1;
}
...
Dialog:HireEmployee(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(IsPlayerConnected(userid) && SQL_IsLogged(userid))
		{
			...
			{
				storeData[index][storeEmploye][i] = pData[userid][pID];
				FurnStore_Save(index);
				Info(playerid, "Kamu telah mempekerjakan "YELLOW"%s "WHITE"ke dalam furniture store milikmu.", ReturnName(userid));
				Info(playerid, "Kamu telah dipekerjakan oleh "YELLOW"%s "WHITE"di furniture store miliknya.", ReturnName(playerid));
				printf("Slot: %d", i);
				return 1;
			}
		}
...
Dialog:FiredEmployee(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(response)
		{
			...
			Info(playerid, "Kamu telah memecat pegawai dari furniture store milikmu.");
...
Dialog:BankSelect(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		Info(playerid, "Kamu telah %s uang sebesar %s %s furniture store.", (type == 1) ? ("menyimpan") : ("mengambil"), FormatMoney(strval(inputtext)), (type == 1) ? ("untuk") : ("dari"));
		FurnStore_Save(index);
...
Dialog:FurnStoreName(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		Info(playerid, "Nama furniture store di ganti menjadi: %s", storeData[ManageFurnStore[playerid]][storeName]);
...
function editLokasiFurn(playerid, objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz, Float:offx, Float:offy, Float:offz)
{
	if(response == EDIT_RESPONSE_CANCEL)
	{
		if(startProduce[playerid])
		{
			...
			InfoTD_MSG(playerid, 2000, "~r~~h~Tidak dapat menekan 'ESC' dalam mode ini");
		}
	}
	if(response == EDIT_RESPONSE_FINAL)
	{
		if(startProduce[playerid])
		{
			if(!IsPointInDynamicArea(production, x, y, z))
				return PlayerEditPoint(playerid, 1489.54, 1789.14, 10.90, 0.0, 0.0, 0.0, "editLokasiFurn", produceObject[playerid]), InfoTD_MSG(playerid, 3000, "~r~~h~Tidak dapat di lakukan di luar area produksi.");

			Info(playerid, "Mulailah memproduksi furniture dengan menahan tombol "RED"'N'");
	...
	if((newkeys & KEY_SECONDARY_ATTACK))
	{
		...
		if((id = FurnStore_Nearest(playerid)) != -1 && IsPlayerInDynamicCP(playerid, storeData[id][storeCP]))
		{
			...
			if (storeData[id][storeLocked])
				return Error(playerid, "This furniture store is locked.");

			Info(playerid, "Gunakan "YELLOW"/buyfurniture "WHITE"untuk membeli furniture");
			SetPlayerPositionEx(playerid, storeData[id][storeIntPos][0], storeData[id][storeIntPos][1], storeData[id][storeIntPos][2], 1000);
		...
		foreach(new kontol : FurnStore) if(IsPlayerInRangeOfPoint(playerid, 2.5, storeData[kontol][storeIntPos][0], storeData[kontol][storeIntPos][1], storeData[kontol][storeIntPos][2]))
		{
			SetPlayerPos(playerid, storeData[kontol][storePos][0], storeData[kontol][storePos][1], storeData[kontol][storePos][2]);
			SetPlayerInterior(playerid, 0);
			SetPlayerVirtualWorld(playerid, 0);
...
function FurnObject_Load(index)
{
	...
	printf("*** [NPRP Database: Loaded] furnobject data (%d count).", cache_num_rows());
	return 1;
}
...
FurnObject_Save(index)
{
	if(Iter_Contains(FurnObject, index))
	{
		...
		format(query, sizeof(query), "UPDATE `furnobject` SET model=%d,name='%e',x=%.3f,y=%.3f,z=%.3f,rx=%.3f,ry=%.3f,rz=%.3f", 
...
FurnObject_Refresh(index)
{
	if(Iter_Contains(FurnObject, index)) 
	{
		...
		FurnStore[index][furnLabel] = CreateDynamic3DTextLabel(sprintf(""GRAY"[ID: %d]\n"YELLOW"%s\n"WHITE"Price: "GREEN"%s\n"WHITE"Stock: "GREEN"%d", index, FurnStore[index][furnName], FormatMoney(FurnStore[index][furnPrice]), FurnStore[index][furnStock]), X11_LIGHTBLUE, FurnStore[index][furnPos][0], FurnStore[index][furnPos][1], FurnStore[index][furnPos][2], 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, (FurnStore[index][furnStoreId]+999), 3);
...
CMD:buyfurniture(playerid, params[])
{
	...
	if((id = FurnStore_Inside(playerid)) != -1)
	{
        ...
        if (FurnStore[i][furnStoreId] == storeData[id][storeID]) {
            ...
            Dialog_Show(playerid, SelectProperty, DIALOG_STYLE_LIST, "Select Property", "House", "Select", "Cancel");
        }
	}
...
Dialog:HouseFurniture(playerid, response, listitem, inputtext[]) {
	if (response) {
		...
		if (storeid != -1) {
            ...
            Custom(playerid, "FURNSTORE", "\"%s"WHITE"\" has arrived to your house", FurnStore[id][furnName]);

            FurnStore[id][furnStock]--;
            storeData[storeid][storeVault] += Server_Percent(price);
    ...
    if(!IsPlayerInAnyVehicle(playerid) && (id = (FurnStore_Inside(playerid) == -1) ? (FurnStore_Nearest(playerid)) : (FurnStore_Inside(playerid))) != -1)
    {
        if(FurnStore_IsOwner(playerid, id) || FurnStore_IsEmployee(playerid, id))
        {
            if(!storeData[id][storeLocked])
            {
                ...
                InfoTD_MSG(playerid, 3000, "You have ~r~locked~w~ the furniture store!");
            ...
            else
            {
                ...
                InfoTD_MSG(playerid, 3000, "You have ~g~unlocked~w~ the furniture store!");
                PlayerPlaySound(playerid, 1145, 0.0, 0.0, 0.0);
            }
        }
    }
    else Error(playerid, "You are not in range of your furniture store.");
    return 1;
}
