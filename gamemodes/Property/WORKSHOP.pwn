...
Workshop_Refresh(id)
{
    if(id > -1)
    {
        ...
        wsData[id][wText] = CreateDynamic3DTextLabel(str, COLOR_GREY, wsData[id][wX], wsData[id][wY], wsData[id][wZ]+0.5, 8.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, -1, 8.0);
...
CMD:editworkshop(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has adjusted the location of Workshop ID: %d.", pData[playerid][pAdminname], wid);
    }
    else if(!strcmp(type, "status", true))
    {
        ...
        if(locked) {
            SendAdminMessage(X11_TOMATO, "%s has Opened Workshop ID: %d.", pData[playerid][pAdminname], wid);
        }
        else {
            SendAdminMessage(X11_TOMATO, "%s has Closed Workshop ID: %d.", pData[playerid][pAdminname], wid);
        }
    }
    else if(!strcmp(type, "price", true))
    {
        ...
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the price of Workshop ID: %d to %d.", pData[playerid][pAdminname], wid, price);
    }
	else if(!strcmp(type, "money", true))
    {
        ...
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the money of Workshop ID: %d to %s.", pData[playerid][pAdminname], wid, FormatMoney(money));
    }
	else if(!strcmp(type, "comp", true))
    {
        ...
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the component of Workshop ID: %d to %d.", pData[playerid][pAdminname], wid, amount);
    }
    else if(!strcmp(type, "mat", true))
    {
        ...
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the material of Workshop ID: %d to %d.", pData[playerid][pAdminname], wid, amount);
    }
    else if(!strcmp(type, "owner", true))
    {
        ...
        Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the owner of Workshop ID: %d to %s", pData[playerid][pAdminname], wid, owners);
    }
    else if(!strcmp(type, "reset", true))
    {
		...
		Workshop_Save(wid);
		Workshop_Refresh(wid);
        SendAdminMessage(X11_TOMATO, "%s has reset Workshop ID: %d.", pData[playerid][pAdminname], wid);
    }
	else if(!strcmp(type, "delete", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has delete Workshop ID: %d.", pData[playerid][pAdminname], wid);
	}
    return 1;
}
...
CMD:myws(playerid)
{
	...
	Loop(itt, (count + 1), 1)
	{
        ...
        if(wid == -1) return Error(playerid, "Anda tak memiliki workshop");
