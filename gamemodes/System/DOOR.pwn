...
Doors_Updatelabel(id)
{
	if(Iter_Contains(Doors, id))
	{
		...
		if(IsValidDynamicCP(dData[id][dCP][0]))
            DestroyDynamicCP(dData[id][dCP][0]);

		if(IsValidDynamicCP(dData[id][dCP][1]))
            DestroyDynamicCP(dData[id][dCP][1]);
		...
		if(dData[id][dGarage] == 1)
		{
			...
			dData[id][dLabelext] = CreateDynamic3DTextLabel(mstr, COLOR_GREY, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ]+0.35, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dData[id][dExtvw], dData[id][dExtint]);
		}
		else
		{
			new mstr[512];
			format(mstr,sizeof(mstr),"[ID: %d]\n{FFFFFF}%s", id, dData[id][dName]);
			dData[id][dPickupext] = CreateDynamicPickup(19130, 23, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtvw], dData[id][dExtint]);
			dData[id][dLabelext] = CreateDynamic3DTextLabel(mstr, COLOR_GREY, dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ]+0.35, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dData[id][dExtvw], dData[id][dExtint]);
			dData[id][dCP][0] = CreateDynamicCP(dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], 1.7, dData[id][dExtvw], dData[id][dExtint], _, 3.0);
		}
        ...
        if(dData[id][dIntposX] != 0.0 && dData[id][dIntposY] != 0.0 && dData[id][dIntposZ] != 0.0)
        {
			if(dData[id][dGarage] == 1)
			{
				...
				dData[id][dLabelint] = CreateDynamic3DTextLabel(mstr, COLOR_GREY, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ]+0.7, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dData[id][dIntvw], dData[id][dIntint]);
			...
			else
			{
				...
				dData[id][dLabelint] = CreateDynamic3DTextLabel(mstr, COLOR_GREY, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ]+0.7, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, dData[id][dIntvw], dData[id][dIntint]);
				dData[id][dPickupint] = CreateDynamicPickup(19130, 23, dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ], dData[id][dIntvw], dData[id][dIntint], -1, 7);
				dData[id][dCP][1] = CreateDynamicCP(dData[id][dIntposX], dData[id][dIntposY], dData[id][dIntposZ], 1.7, dData[id][dIntvw], dData[id][dIntint], _, 3.0);
			}
		}
	}
}
...
CMD:gotodoor(playerid, params[])
{
	...
	format(fmt, sizeof fmt, "ID\tDoor Name\n");
	foreach(new i : Doors) if(Iter_Contains(Doors, i))
	{
		format(fmt, sizeof fmt, "%s%d\t%s\n", fmt, i, dData[i][dName]);
	}
	Dialog_Show(playerid, DoorGoto, DIALOG_STYLE_TABLIST_HEADERS, "Door Goto", fmt, "Goto", "Cancel");
	/*
...
CMD:editdoor(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has adjusted the location of entrance ID: %d.", pData[playerid][pAdminname], did);
    }
    else if(!strcmp(type, "interior", true))
    {
       ...
       /*foreach (new i : Player)
        ...
        SendAdminMessage(X11_TOMATO, "%s has adjusted the interior spawn of entrance ID: %d.", pData[playerid][pAdminname], did);
    }
    else if(!strcmp(type, "custom", true))
    {
        ...
        if(status) {
            SendAdminMessage(X11_TOMATO, "%s has enabled custom interior mode for entrance ID: %d.", pData[playerid][pAdminname], did);
        }
        else {
            SendAdminMessage(X11_TOMATO, "%s has disabled custom interior mode for entrance ID: %d.", pData[playerid][pAdminname], did);
        }
    }
    else if(!strcmp(type, "virtual", true))
    {
        ...
        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the virtual of entrance ID: %d to %d.", pData[playerid][pAdminname], did, worldid);
    }
    else if(!strcmp(type, "password", true))
    {
        ...
        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(X11_TOMATO, "%s has adjusted the password of entrance ID: %d to %s", pData[playerid][pAdminname], did, password);
    }
    else if(!strcmp(type, "locked", true))
    {
        ...
        if(locked) {
            SendAdminMessage(X11_TOMATO, "%s has locked entrance ID: %d.", pData[playerid][pAdminname], did);
        } else {
            SendAdminMessage(X11_TOMATO, "%s has unlocked entrance ID: %d.", pData[playerid][pAdminname], did);
        }
    }
    else if(!strcmp(type, "name", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has adjusted the name of entrance ID: %d to \"%s\".", pData[playerid][pAdminname], did, ColouredText(name));
    }
	else if(!strcmp(type, "admin", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has set entrance ID: %d to admin level %d.", pData[playerid][pAdminname], did, level);
    }
	else if(!strcmp(type, "vip", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has set entrance ID: %d to VIP level %d.", pData[playerid][pAdminname], did, level);
    }
	else if(!strcmp(type, "faction", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has set entrance ID: %d to faction id %d.", pData[playerid][pAdminname], did, fid);
    }
	else if(!strcmp(type, "family", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has set entrance ID: %d to family id %d.", pData[playerid][pAdminname], did, fid);
    }
	else if(!strcmp(type, "garage", true))
	{
		...
		{
			dData[did][dGarage] = 0;
			SendAdminMessage(X11_TOMATO, "%s has set entrance ID: %d to garage vehicle disable.", pData[playerid][pAdminname], did);
		}
		else
		{
			dData[did][dGarage] = 1;
			SendAdminMessage(X11_TOMATO, "%s has set entrance ID: %d to garage vehicle enable.", pData[playerid][pAdminname], did);
		}
		Doors_Save(gid);
	...
	else if(!strcmp(type, "delete", true))
    {
		...
		DestroyDynamicCP(dData[did][dCP][0]);
		DestroyDynamicCP(dData[did][dCP][1]);
        ...
        SendAdminMessage(X11_TOMATO, "%s has delete door ID: %d.", pData[playerid][pAdminname], did);
	}
    return 1;
}