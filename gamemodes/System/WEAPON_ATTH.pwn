...
PickupWeapon(playerid, itemid)
{
    if(itemid != -1 && DropWeap[itemid][WeapModel])
    {
		...
		Servers(playerid, "Anda telah mengambil senjata "YELLOW"%s", ReturnWeaponName(DropWeap[itemid][WeaponID]));
        DeleteWeapon(itemid);
    }
    return 1;
}
...
CMD:weapon(playerid, params[])
{
	...
	else if(!strcmp(name, "give", true))
	{
		...
		Info(playerid, "Anda telah memberikan weapon "YELLOW"%s "YELLOW"kepada "LIGHTGREEN"%s.", ReturnWeaponName(weaponid) , ReturnName(otherid));
		Info(otherid, ""LIGHTGREEN"%s "WHEAT"telah memberikan weapon "YELLOW"%s "WHITE"kepada anda.", ReturnName(playerid), ReturnWeaponName(weaponid));
