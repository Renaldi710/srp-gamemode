...
CMD:removeatm(playerid, params[])
{
	...
	DestroyDynamicCP(AtmData[id][atmCP]);
	...
	AtmData[id][atmObjID] = AtmData[id][atmCP] = -1;
...
CMD:atm(playerid, params[])
{
    ...
    foreach(new i : ATMS) 
    {
        if(IsPlayerInDynamicCP(playerid, AtmData[i][atmCP]))
        {
            count++;
            new tstr[128];
            ...
            break; // keluar loop setelah ketemu satu ATM
        }
    }

    if(count < 1)
        return Error(playerid, "Anda tidak berada di ATM");
