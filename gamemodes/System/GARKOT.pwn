	...
	if(rows)
  	{
		...
		while(i < rows)
		{
			...
			format(str, sizeof(str), "[ID: %d]\n"WHITE"Park Location: "YELLOW"%s\n"YELLOW"/storepv "WHITE_E"- untuk memarkirkan kendaraan\n"YELLOW"/takepv "WHITE_E"- untuk mengambil kendaraan", id, GetLocation(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]));
			...
			ppData[id][parkLabel] = CreateDynamic3DTextLabel(str, COLOR_GREY, ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]+0.3, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, ppData[id][parkWorld], ppData[id][parkInt], -1, 10.0);
			Iter_Add(Parks, id);
	    	i++;
		}
