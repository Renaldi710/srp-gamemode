...
CMD:report(playerid, params[])
{
    ...
    {
        Custom(playerid, "REPORT", "Laporan anda: {FFFF00}%s", params);
        SendStaffMessage(COLOR_YELLOW, "[REPORT #%d] {FFFF90}%s(%d): "WHITE"%s", reportid, pData[playerid][pName], playerid, params);
...
CMD:dr(playerid, params[])
{
    ...
    SendStaffMessage(X11_TOMATO, "[DENIED]{FFFF90}[%d] "RED"%s {FFFF90} has denied %s(%d) report.", pData[playerid][pAdminname], pData[ReportData[reportid][rPlayer]][pName], ReportData[reportid][rPlayer]);
    Custom(ReportData[reportid][rPlayer], "[DENY REPORT]",  "{FF0000}%s {FFFFFF}denied your report: %s.", pData[playerid][pAdminname], msg);
...
CMD:clearreports(playerid, params[])
{
    ...
    SendStaffMessage(X11_TOMATO, "AdmCmd: %s has removed all reports on the server.", pData[playerid][pAdminname]);
    return 1;
}