...
Family_Refresh(id)
{
	if(id != -1)
	{
		...
		format(mstr,sizeof(mstr),"[ID: %d]\n{FFFFFF}Family Name: "GRAY"%s", id, fData[id][fName]);
		...
		if(fData[id][fSafeposX] != 0.0 && fData[id][fSafeposY] != 0.0 && fData[id][fSafeposZ] != 0.0)
		{
			...
			fData[id][fLabelsafe] = CreateDynamic3DTextLabel(tstr, -1, fData[id][fSafeposX], fData[id][fSafeposY], fData[id][fSafeposZ]+0.7, 5.0);
...
CMD:fcreate(playerid, params[])
{
	...
	SendStaffMessage(X11_TOMATO, "AdmCmd: Admin %s telah membuat family ID: %d dengan leader: %s", pData[playerid][pAdminname], fid, pData[otherid][pName]);
...
CMD:fdelete(playerid, params[])
{
	...
	mysql_tquery(g_SQL, query);
    SendStaffMessage(X11_TOMATO, "AdmCmd: Admin %s telah menghapus family ID: %d.", pData[playerid][pAdminname], fid);
	return 1;
}
...
CMD:fedit(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendStaffMessage(X11_TOMATO, "AdmCmd: %s has adjusted the location of entrance ID: %d.", pData[playerid][pAdminname], fid);
    }
    else if(!strcmp(type, "interior", true))
    {
        ...
        SendStaffMessage(X11_TOMATO, "AdmCmd: %s has adjusted the interior spawn of entrance ID: %d.", pData[playerid][pAdminname], fid);
    }
    else if(!strcmp(type, "name", true))
    {
        ...
        SendStaffMessage(COLOR_LRED, "AdmCmd: Admin %s has changed the family name ID: %d to: %s.", pData[playerid][pAdminname], fid, name);
    }
    else if(!strcmp(type, "leader", true))
    {
        ...
        SendStaffMessage(COLOR_LRED, "AdmCmd: Admin %s has changed the family leader ID: %d to: %s.", pData[playerid][pAdminname], fid, pData[otherid][pName]);
    }
    else if(!strcmp(type, "safe", true))
    {
		...
		SendStaffMessage(COLOR_LRED, "AdmCmd: Admin %s has changed the family safepos ID: %d.", pData[playerid][pAdminname], fid);
    }
    else if(!strcmp(type, "money", true))
    {
		...
		SendStaffMessage(COLOR_LRED, "AdmCmd: Admin %s has changed the family money ID: %d to %s.", pData[playerid][pAdminname], fid, FormatMoney(money));
    }
    else if(!strcmp(type, "marijuana", true))
    {
		...
		SendStaffMessage(COLOR_LRED, "AdmCmd: dmin %s has changed the family marijuana ID: %d to %s.", pData[playerid][pAdminname], fid, marijuana);
    }
	else if(!strcmp(type, "component", true))
    {
		...
		SendStaffMessage(COLOR_LRED, "AdmCmd: Admin %s has changed the family component ID: %d to %s.", pData[playerid][pAdminname], fid, comp);
    }
	else if(!strcmp(type, "material", true))
    {
		...
		SendStaffMessage(COLOR_LRED, "AdmCmd: Admin %s has changed the family material ID: %d to %s.", pData[playerid][pAdminname], fid, mat);
    }
    return 1;
}
...
CMD:finvite(playerid, params[])
{
	...
	Servers(playerid, "Anda telah menginvite "YELLOW"%s "WHITE"untuk menjadi anggota family.", pData[otherid][pName]);
	Servers(otherid, ""YELLOW"%s "WHITE"telah menginvite anda untuk menjadi anggota family. Type: /accept family or /deny family!", pData[playerid][pName]);
	return 1;
}
...
CMD:funinvite(playerid, params[])
{
	...
	Servers(playerid, "Anda telah mengeluarkan "YELLOW"%s "WHITE"dari anggota family.", pData[otherid][pName]);
	Servers(otherid, ""YELLOW"%s "WHITE"telah mengeluarkan anda dari anggota family.", pData[playerid][pName]);
	return 1;
}
...
CMD:fsetrank(playerid, params[])
{
	...
	Servers(playerid, "You has set "YELLOW"%s "WHITE"family rank to level %d", pData[otherid][pName], rank);
	Servers(otherid, ""YELLOW"%s "WHITE"has set your family rank to level "YELLOW"%d", pData[playerid][pName], rank);
	return 1;
}
