...
Locker_Refresh(lid)
{
    if(lid != -1)
    {
		...
		format(string, sizeof(string), "[ID: %d]\n"WHITE_E"Locker Type: "YELLOW_E"%s\n"WHITE_E"Type '"RED_E"/locker"WHITE_E"' to open", lid, type);
		lData[lid][lPickup] = CreateDynamicPickup(1239, 23, lData[lid][lPosX], lData[lid][lPosY], lData[lid][lPosZ]+0.2, -1, lData[lid][lInt], _, 4);
		lData[lid][lLabel] = CreateDynamic3DTextLabel(string, COLOR_GREY, lData[lid][lPosX], lData[lid][lPosY], lData[lid][lPosZ]+0.5, 2.5);
	}
    return 1;
}
...
CMD:editlocker(playerid, params[])
{
    ...
    if(!strcmp(type, "location", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has adjusted the location of locker ID: %d.", pData[playerid][pAdminname], lid);
    }
    else if(!strcmp(type, "type", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has set locker ID: %d to type id faction %d.", pData[playerid][pAdminname], lid, tipe);
    }
    else if(!strcmp(type, "delete", true))
    {
        ...
        SendAdminMessage(X11_TOMATO, "%s has delete locker ID: %d.", pData[playerid][pAdminname], lid);
    }
    return 1;
}